...
#ifndef MODVM_COMPILER_CONTEXT_ANALYSIS_H
#define MODVM_COMPILER_CONTEXT_ANALYSIS_H
...
#define context_lock_struct(name, ...)                                         \
	...
	static __always_inline void __acquire_shared_ctx_lock(                 \
		const struct name *var)                                        \
		__attribute__((overloadable)) __no_context_analysis            \
			__acquires_shared_ctx_lock(var)                        \
	{                                                                      \
	}                                                                      \
	static __always_inline bool __try_acquire_ctx_lock(                    \
		const struct name *var, bool ret)                              \
		__attribute__((overloadable)) __no_context_analysis            \
			__try_acquires_ctx_lock(1, var)                        \
	{                                                                      \
		return ret;                                                    \
	}                                                                      \
	static __always_inline bool __try_acquire_shared_ctx_lock(             \
		const struct name *var, bool ret)                              \
		__attribute__((overloadable)) __no_context_analysis            \
	...
	static __always_inline void __release_shared_ctx_lock(                 \
		const struct name *var)                                        \
		__attribute__((overloadable)) __no_context_analysis            \
	...
	static __always_inline void __assume_shared_ctx_lock(                  \
		const struct name *var)                                        \
...
#define __requires_ctx_lock(...)
#define __requires_shared_ctx_lock(...)
#define __acquire_ctx_lock(var) \
	do {                    \
	} while (0)
#define __acquire_shared_ctx_lock(var) \
	do {                           \
	} while (0)
#define __try_acquire_ctx_lock(var, ret) (ret)
#define __try_acquire_shared_ctx_lock(var, ret) (ret)
#define __release_ctx_lock(var) \
	do {                    \
	} while (0)
#define __release_shared_ctx_lock(var) \
	do {                           \
	} while (0)
#define __assume_ctx_lock(var) \
	do {                   \
		(void)(var);   \
	} while (0)
#define __assume_shared_ctx_lock(var) \
	do {                          \
		(void)(var);          \
	} while (0)
...
#define __cond_acquires_impl_true(x, ...) \
	__try_acquires##__VA_ARGS__##_ctx_lock(1, x)
#define __cond_acquires_impl_false(x, ...) \
	__try_acquires##__VA_ARGS__##_ctx_lock(0, x)
...
#endif /* MODVM_COMPILER_CONTEXT_ANALYSIS_H */